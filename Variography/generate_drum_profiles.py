...
def main() -> None:
    ...
    for idx, profile in enumerate(profiles, start=1):
        csv_path = output_dir / f"drum_profile_{idx:02d}.csv"
        mat_path = output_dir / f"drum_profile_{idx:02d}.mat"
        profile.save_csv(csv_path)
        savemat(
            mat_path,
            {
                "time_s": profile.t,
                "drum_angle_deg": profile.theta_deg,
                "drum_velocity_deg_s": profile.v_deg_s,
                "drum_acceleration_deg_s2": profile.a_deg_s2,
            },
        )
